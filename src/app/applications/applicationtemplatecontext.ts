...
export class ApplicationTemplateContext {
    ...
    private static getSubComponentRecursive(component: ApplicationComponent, componentId: string, container?: boolean, includeIndex?: boolean): ApplicationComponent | FoundComponent {
        ...
        if (component.isComposite) {
            ...
            for (let sub of components) {
                found = this.getSubComponentRecursive(sub, componentId, container);

                if (found != null) {
                    if (includeIndex) {
                        const foundComponent: ApplicationComponent = (found instanceof FoundComponent) ? found.component : found;
                        return new FoundComponent(components, foundComponent, index);
                    } else {
                        return found;
                    }
                }
    ...
    getSubComponent(application: string, componentId: string, container?: boolean, includeIndex?: boolean): ApplicationComponent | FoundComponent {
        ...
        if (template) {
            ...
            for (let component of template.components) {
                found = ApplicationTemplateContext.getSubComponentRecursive(component, componentId, container, includeIndex);
...
export class FoundComponent {
     ...
     */
    constructor(array: ApplicationComponent[], component: ApplicationComponent, index: number) {
        this.array = array;
